...
def collapse_results_by_party(results_by_candidate, candidates):
    results_by_party = {}
    for candidate, count in results_by_candidate.items(): # iteritems for Python 2
...
records = []
for state, state_results in all_results.items(): # iteritems() for Python 2
...
time_series_df.to_csv(f'nyt_ts.csv', encoding='utf-8')
